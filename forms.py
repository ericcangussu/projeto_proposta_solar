...
def escolha_tipo_proposta():
    st.subheader('Escolha do Tipo de Proposta')
    st.session_state['tipo_proposta'] = st.selectbox(
        'Selecione o tipo de proposta:',
        ['Selecione', 'Subestação Unitária', 'Estação Fotovoltaica']
    )

def configuracao_itens():
    if st.session_state['tipo_proposta'] != 'Selecione':
        st.subheader(f'Configuração da {st.session_state["tipo_proposta"]}')
        ...
        if tipo_configuracao == 'SKID (Transformador Isolado + QBGT)':
            ...
            configurar_qgbt_e_trafo()
        if st.session_state['tipo_proposta'] == 'Estação Fotovoltaica':
            configurar_itens_adicionais()
    
    # Monta a tabela de visualização dos itens configurados
    visualizar_tabela()
...
def configurar_itens_adicionais():
